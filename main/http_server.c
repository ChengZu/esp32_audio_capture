...
static esp_err_t ota_handler(httpd_req_t *req)
{
    ...
    static int frame = 0;
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT &&
        strcmp((char *)ws_pkt.payload, "Start upload ota") == 0)
    {
        frame = 0;
        ota_begin();
        msg = "Start ota";
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT &&
        strcmp((char *)ws_pkt.payload, "Data end") == 0)
    {
        frame = 0;
        msg = "Ota end";
...
static const httpd_uri_t ws_ota = {
    .uri = "/ota",
    .method = HTTP_GET,
    .handler = ota_handler,
    .user_ctx = NULL,
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(server, &ws_ota);
#if CONFIG_EXAMPLE_ENABLE_SSE_HANDLER
